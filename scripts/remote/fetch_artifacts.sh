#!/usr/bin/env bash
# Rsync a relative path from the Spartan PGSR tree to a local directory.
...
# multi-hundred-MB path here, inspect it in place with `ssh spartan` instead.
set -euo pipefail

SPARTAN_HOST="${SPARTAN_HOST:-spartan}"
REMOTE_ROOT="${REMOTE_ROOT:-/data/gpfs/projects/punim2657/PGSR}"
